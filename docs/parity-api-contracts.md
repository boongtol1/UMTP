...
조사일: 2026-09-15. 기준은 작업 시작 시 저장소 코드이며 기준 SHA/브랜치와 구현·검증 상태의 최종 집계는 [기능 대응표](android-ios-parity.md)를 따른다. API 표의 “iOS 없음”은 변경 전 baseline 차이로, 현재 활성 iOS 기능의 누락을 뜻하지 않는다. C15 commit 후 전체106개(단위86+UI20)가 실패0·skip0·exit0으로 통과했으며 로컬 구현·검증을 완료했다. 정적 계약 조사와 fixture 검증은 실서비스 요청/DB 반영 성공을 의미하지 않는다.
...
- Base URL은 Android `BuildConfig.UMTP_BASE_URL`, 기본 HTTPS DuckDNS. iOS 설정값은 `App/AppConfig.swift`. Android 주 API client는 timeout을 명시하지 않아 OkHttp 기본값을 사용하며, DoH 조회 전용 client는 call/connect/read를 4초로 지정한다. 변경 전 iOS는 시스템 DNS만 사용했다. 현재 C4 `DNSRecovery`는 허용 HTTPS 호스트의 DNS 실패에서 native encrypted DNS 복구를 사용하며 원본 URL/TLS와 전송된 쓰기 재시도 금지를 유지한다. 실제 iOS 장애망/선택 resolver 검증은 별도다.
...
| ID | Android 심볼 / HTTP 경로 | 입력 | 결과·예외·서버 근거 | 활성 / 변경 전 iOS 차이 |
|---|---|---|---|---|
...
| API27 | markAlertEventReadPost / POST alert-events/{id}/read | API26 동일 | API26 동일 | API26 HTTP404/405/501만 fallback |
| API28 | markAllAlertEventsRead / PATCH alert-events/read-all | query user_id | updated_count, :952 | 활성 모두 읽음; iOS 없음 |
| API29 | markAllAlertEventsReadPost / POST alert-events/read-all | API28 동일 | API28 동일 | API28 HTTP404/405/501만 fallback |
...
- 서버 purchase PATCH는 정확확인/구매가 수정 시 기본적으로 INSPECTED로 되돌린다. 기존 KEEP/RESALE_LISTED/SOLD 거래의 구매 정보 수정 및 되팔이 정확확인 보조 저장에서는 현재 단계를 명시적으로 보존하여 단계가 의도치 않게 퇴행하지 않도록 한다. 사용자가 현재 단계를 직접 변경하면 그 입력이 우선이다.

### 구현 중 확인한 계약 보완 (C8~C10)

- 새 되팔이 draft의 명시 단계는 legacy upsert 내부의 두 번째 PATCH에서 덮일 수 있다. `resale_trade_journeys.py:2714` 이후 필드 분리·`:2725` resale PATCH·`:2732` sold 또는 `:2739` 빈 resale PATCH와 `:1717` 단계 파생이 근거다. iOS는 명시 단계가 있는 신규 되팔이 저장만 동일 identity의 빈 purchase upsert로 양수 ID를 확보한 뒤 한 번의 resale PATCH로 전체 변경을 보낸다. 두 번째 요청 실패를 성공으로 표시하지 않으며 같은 user/source/product identity 재시도로 중복 생성을 막는다. 실제 함수의 메모리 실행에서 KEEP+판매금액→SOLD, KEEP+플랫폼→RESALE_LISTED 덮임을 확인했으며 실서비스 쓰기는 하지 않았다.
- 정확 확인만 바꾼 되팔이 저장은 보조 purchase 저장 성공 후 변경된 되팔이 필드가 없을 때만 빈 resale PATCH를 생략한다(C9). 확인정보 변경 없는 명시적 빈 저장은 기존 계약을 유지한다.
- 재판매 URL은 이미지용 JSON 배열/객체 파서를 사용하지 않고 단일 HTTP(S) 주소·host·공백을 검증한다. 날짜는 공백+offset/Z/소수초를 지원하되 불가능한 달력 날짜와 시/분/초는 거부하고 유효 원본 입력을 보존한다(C8/C10).
- API20/21은 삭제 ID가 아닌 `deleted_count`만 반환한다. 선택 요청 수와 삭제 수가 정확히 같은 경우에만 해당 선택 초안을 해제한다. 0건/부분/전체 삭제는 원래 양수 ID와 입력을 보존하고 최신 기록 확인을 안내한다. 최대200개 내역의 부재는 삭제 증거가 아니며 후속 저장은 원래 ID의 PATCH를 유지하여 묵시적 재생성을 하지 않는다(C10).

위 요청 순서·부분 실패·단계·strict URL·삭제/날짜 회귀는 Trade27에 포함해 R-FINAL3에서 통과했다. C15의 loopback fixture 설정은 Debug Simulator 자동화 격리이며 운영 API 계약을 변경하지 않는다. mock DNS 단위는 명시적인 HTTPS 호스트와 주입 transport를 사용해 fixture 기본 URL과 분리한다.
...
5. 앱 외 Notification Listener/URL scheme/Universal Links는 Android Manifest에 실제 활성 구현이 없으므로 README 대기 항목을 완료 기능으로 취급하지 않는다. 실제 `MainActivity.handleIntent`는 양수 `alert_id`만 사용해 알림 화면으로 이동한다. URL deep link가 아니다.
