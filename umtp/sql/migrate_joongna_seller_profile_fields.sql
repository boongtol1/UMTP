...
SELECT COUNT(*) INTO @has_search_results_table
...
SET @sql_search_results_seller_store_seq = IF(
  @has_search_results_table = 0,
  'SELECT "skip search_results.seller_store_seq"',
  IF(
...
SET @sql_search_results_seller_store_name = IF(
  @has_search_results_table = 0,
  'SELECT "skip search_results.seller_store_name"',
  IF(
...
SET @sql_search_results_seller_profile_image_url = IF(
  @has_search_results_table = 0,
  'SELECT "skip search_results.seller_profile_image_url"',
  IF(
...
SET @sql_search_results_seller_store_level = IF(
  @has_search_results_table = 0,
  'SELECT "skip search_results.seller_store_level"',
  IF(
...
SET @sql_search_results_seller_trust_score = IF(
  @has_search_results_table = 0,
  'SELECT "skip search_results.seller_trust_score"',
  IF(
...
SET @sql_search_results_seller_review_count = IF(
  @has_search_results_table = 0,
  'SELECT "skip search_results.seller_review_count"',
  IF(
...
SELECT COUNT(*) INTO @has_alert_events_seller_store_seq
FROM INFORMATION_SCHEMA.COLUMNS
WHERE TABLE_SCHEMA = @target_db
  AND TABLE_NAME = 'alert_events'
  AND COLUMN_NAME = 'seller_store_seq';
SET @sql_alert_events_seller_store_seq = IF(
  @has_alert_events_table = 0,
  'SELECT "skip alert_events.seller_store_seq"',
  IF(
    @has_alert_events_seller_store_seq = 0,
    'ALTER TABLE alert_events ADD COLUMN seller_store_seq BIGINT NULL AFTER product_id',
    'SELECT "alert_events.seller_store_seq exists"'
  )
);
PREPARE stmt_alert_events_seller_store_seq FROM @sql_alert_events_seller_store_seq;
EXECUTE stmt_alert_events_seller_store_seq;
DEALLOCATE PREPARE stmt_alert_events_seller_store_seq;

SELECT COUNT(*) INTO @has_alert_events_seller_store_name
FROM INFORMATION_SCHEMA.COLUMNS
WHERE TABLE_SCHEMA = @target_db
  AND TABLE_NAME = 'alert_events'
  AND COLUMN_NAME = 'seller_store_name';
SET @sql_alert_events_seller_store_name = IF(
  @has_alert_events_table = 0,
  'SELECT "skip alert_events.seller_store_name"',
  IF(
    @has_alert_events_seller_store_name = 0,
    'ALTER TABLE alert_events ADD COLUMN seller_store_name VARCHAR(100) NULL AFTER seller_store_seq',
    'SELECT "alert_events.seller_store_name exists"'
  )
);
PREPARE stmt_alert_events_seller_store_name FROM @sql_alert_events_seller_store_name;
EXECUTE stmt_alert_events_seller_store_name;
DEALLOCATE PREPARE stmt_alert_events_seller_store_name;
