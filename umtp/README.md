...
- 기존 `/silicon-mac-units`, `/macbook-air-units`, 사용자 설정·감시 조건·개별/일괄 저장·초기화 경로에 포함됩니다. 신규 조건은 감시 OFF이고, 저장한 사용자 시장가·검색어·우선순위가 우선합니다. 칩이나 세부 사양을 붙이지 않은 기본 검색어 `맥북 네오`를 사용합니다.
- 제목·본문·자가점검 사양의 한글/영문 제품명과 `A18 Pro` 칩을 처리합니다. 칩이 생략되어도 제목 또는 구조화 모델명이 네오 본체임을 확인할 수 있으면 카탈로그의 유일 칩을 사용하며, 구조화 `CPU종류=A18`도 같은 제품 근거로 정규화합니다. 추론 여부와 원래 값은 파싱 근거에 남기고 신뢰도를 낮춥니다. 이 추론 경로에서는 SSD가 명시되거나 `기본형/깡통` 근거가 있어야 하며, 미지원·상충 사양이나 액세서리/구매글은 임의로 보정하지 않습니다. 명시된 `A18 Pro`의 화면·RAM·SSD 기본형 처리와 칩의 `18`을 RAM으로 오인하지 않는 규칙은 유지합니다.
...
- 맥북 프로·네오·아이맥·맥 스튜디오는 제목과 본문을 구분하여 같은 파서로 분석합니다. 구조화 CPU가 `M1` 같은 계열명만 담은 경우, 제목/모델명의 명시적인 칩 등급 또는 해당 제품 카탈로그의 유일한 후보가 있을 때만 구체화합니다. 여러 후보나 상충된 칩은 추측하지 않습니다.
- 제목과 구조화 SSD가 모두 같은 용량을 명시할 때만, 본문의 `(256GB는 안돼요)` 같은 제한적인 비교 부연을 사양 추출에서 제외합니다. 실제 판매 사양의 모순이나 복수 제품/용량은 계속 거절하며, 원본 본문은 변경하지 않습니다.
- 실제 `watch_rule_id`에 연결된 제품·칩·화면·RAM·SSD, 사용자, 활성 여부, 설정 저장시각, 가격 방향과 범위를 통과해야 알림을 만듭니다. 분석 성공이 곧 알림 발송을 의미하지 않으며, 사용자 설정 가격을 넘는 매물은 보내지 않습니다.
- 이번 보정은 DB 마이그레이션이나 가격 시드 재실행이 필요하지 않습니다. 서버 이미지 재빌드 후 API·polling·analysis·content-refresh 컨테이너에 같은 코드를 반영하세요. 기존에 분석 완료된 글을 일괄 재전송하지 않으며, 새 글 또는 변경된 글부터 기존 중복 방지 정책에 따라 처리합니다.
...
- 1.8 immediate enqueue 옵션: `ENABLE_IMMEDIATE_ANALYSIS_ENQUEUE=true`면 검색 결과 매칭 직후 `analysis_jobs`를 선등록합니다. 변경 감지 reason(`new/sort_date_changed/price_changed/...`)은 가능한 그대로 유지하고, `unchanged` 보강 경로에서만 `trigger_reason=immediate_search_result`를 사용합니다. 이 경로는 `unchanged_backfill`을 제거하는 대체 로직이 아니라, 누락과 지연을 줄이기 위한 빠른 보조 경로입니다.
...
- 앱 문구 정리: 설정 화면 차이율 계산식은 `user_id` 기반으로 표시하고(없으면 `사용자` fallback), 알림피드/읽음 보관함의 계산식 설명과 과도한 안내 문구는 제거했습니다.
- iOS Stage1 진행: 앱 시작 시 `user_id` 세션 복원 → 미보유 시 등록 화면 → 등록 성공 시 세션 저장 후 메인 진입 흐름을 연결했습니다.
- iOS Stage1 세션 저장소: `UserDefaults` 기반 `user_id` 저장/복원/삭제를 사용합니다.
- iOS Stage1 API 연결: `POST /users/register`를 iOS `UserAPI`에 연결했고 요청에 `user_id`, `device_id`, `platform=ios`를 포함합니다.
- iOS Stage2 예정: Keychain 기반 영구 식별자, Alert Feed/Settings 화면, Push 알림 연동을 추가할 예정입니다.
