...
DEFAULT_FAIR_PRICE_KRW = 550000
ALERT_THRESHOLD_RATIO = 20.0

FAKE_LISTING = {
    "title": "맥북에어 M1 8GB 256GB 팝니다",
    "product_type": "MacBook Air",
    "chip": "M1",
    "screen_inch": 13,
    "ram_gb": 8,
    "ssd_gb": 256,
    "listing_price_krw": 440000,
}
...
def insert_default_fair_price(cursor, listing):
    cursor.execute(
        """
        INSERT INTO mac_fair_prices (
            product_type, chip, screen_inch, ram_gb, ssd_gb, fair_price_krw
        )
        VALUES (%s, %s, %s, %s, %s, %s)
        ...
        (
            ...
            listing["ram_gb"],
            listing["ssd_gb"],
            DEFAULT_FAIR_PRICE_KRW,
        ),
    )


def save_analysis_result(cursor, listing, fair_price_krw, diff_amount_krw, diff_ratio, is_alert_target):
    cursor.execute(
        """
...
def main():
    ...
    try:
        ...
        if fair_price_krw is None:
            insert_default_fair_price(cursor, listing)
            connection.commit()
            fair_price_krw = fetch_fair_price(cursor, listing)

        if fair_price_krw is None:
            raise RuntimeError("공정가 저장 또는 조회에 실패했습니다.")
